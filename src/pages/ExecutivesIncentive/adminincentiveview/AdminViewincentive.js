...
const AdminViewincentive = () => {
  ...
  const uniquePaymentModes = ['All', ...Array.from(new Set(incentives.map(item => item.ModeofPayment)))];

  const formatNumberWithCommas = (num) => {
    return num.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  };

  const applyFilters = (exe, month, IncentiveStatus, Incentivesettlement, customer, start = startDate, end = endDate, paymentMode = selectedPaymentMode) => {
    ...
    // ✅ Payment Mode Filter (Cheque / Cash)
if (paymentMode !== 'All') {
  filtered = filtered.filter(item =>
    item.ModeofPayment &&
        ...
        <div className="incentive-container">
          ...
          <div className="no-print" style={{ marginBottom: '10px' }}>
            ...
            <input
              ...
              style={{ padding: '5px', width: '180px', marginRight: '10px' }}
            />
            <label style={{ marginRight: '8px' }}>Payment Mode:</label>
<select
  value={selectedPaymentMode}
  onChange={handlePaymentModeChange}
  style={{ padding: '5px', width: '180px', marginRight: '10px' }}
>
  {uniquePaymentModes.map((mode, idx) => (
